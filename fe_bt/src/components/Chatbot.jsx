...
const Chatbot = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const botMessage = {
                type: 'bot',
                content: tour.tourName ? botMessageContent : 'Không tìm thấy tour phù hợp với yêu cầu của bạn.',
                tourId: tour.tourId,
                timestamp: new Date().toLocaleTimeString('vi-VN', { hour: '2-digit', minute: '2-digit' })
            };
            setMessages((prev) => [...prev, botMessage]);
        } catch (err) {
            const errorMessage = err.response?.data?.Message || 'Đã xảy ra lỗi khi lấy gợi ý tour.';
            const botMessage = {
                type: 'bot',
    ...
    const handleCopyMessage = (content) => {
        navigator.clipboard.writeText(content.replace(/<span class="highlight-[^"]*">|<\/span>/g, '').replace(/\*\*.*?\*\*: /g, '').replace(/\n/g, '\n')).then(() => {
                    ...
                    <div className="chatbot-content" ref={chatContentRef}>
                        {messages.length === 0 && (
                            <div className="chatbot-welcome">
                                Chào bạn! Hãy nhập yêu cầu (ví dụ: "Tôi muốn đi Đà Lạt") để tôi gợi ý tour nhé!
                            </div>
                        )}
